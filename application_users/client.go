...
func (c *Client) list(ctx context.Context, arg *ngrok.Paging) (*ngrok.ApplicationUserList, error) {
	if arg == nil {
		arg = new(ngrok.Paging)
	}
	var res ngrok.ApplicationUserList
	var path bytes.Buffer
	if err := template.Must(template.New("list_path").Parse("/app/users")).Execute(&path, arg); err != nil {
		panic(err)
	}
	var (
		apiURL  = &url.URL{Path: path.String()}
...
func (it *Iter) Item() *ngrok.ApplicationUser {
