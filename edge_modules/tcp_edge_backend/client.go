...
func (c *Client) Replace(ctx context.Context, arg *ngrok.EdgeBackendReplace) (*ngrok.EndpointBackend, error) {
	...
	if err := template.Must(template.New("replace_path").Parse("/edges/tcp/{{ .ID }}/backend")).Execute(&path, arg); err != nil {
...
func (c *Client) Get(ctx context.Context, id string) (*ngrok.EndpointBackend, error) {
	...
	var res ngrok.EndpointBackend
	var path bytes.Buffer
	if err := template.Must(template.New("get_path").Parse("/edges/tcp/{{ .ID }}/backend")).Execute(&path, arg); err != nil {
...
func (c *Client) Delete(ctx context.Context, id string) error {
	...
	var path bytes.Buffer
	if err := template.Must(template.New("delete_path").Parse("/edges/tcp/{{ .ID }}/backend")).Execute(&path, arg); err != nil {
